...
<ul class="product_list">
    <?php while ($row = mysqli_fetch_array($query_pro)) { ?>
        <li>
            <a class="product-card-link" href="index.php?quanly=sanpham&id=<?php echo $row['id_sanpham'] ?>">
                <?php
                    $fn = trim($row['hinhanh']);
            ...
            <div class="product-card-foot">
                <p class="category_product"><?php echo htmlspecialchars($row['tendanhmuc']) ?></p>
                <form class="product-add-form js-add-to-cart-form" method="POST" action="pages/main/themgiohang.php">
                    <input type="hidden" name="themgiohang" value="1">
                    <input type="hidden" name="idsanpham" value="<?php echo (int)$row['id_sanpham']; ?>">
                    <button class="product-add-btn" type="submit" aria-label="Thêm vào giỏ hàng">
