...
// Ensure cancel_requested column exists (best-effort)
$ensureCancelRequestedColumn = function () use ($mysqli) {
    try {
...
// Ensure order_status column exists (best-effort)
$ensureOrderStatusColumn = function () use ($mysqli) {
    try {
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['code'])) {
    ...
    $ensureCancelRequestedColumn();

    if ($order_status === 5) {
        $stmt = $mysqli->prepare("UPDATE table_giohang SET order_status=?, cancel_requested=0 WHERE code_cart=?");
    } else {
...
} elseif (isset($_GET['action']) && $_GET['action'] === 'approve_cancel' && isset($_GET['code'])) {
    $code_cart = $_GET['code'];
    $ensureOrderStatusColumn();
    $ensureCancelRequestedColumn();

    $stmt = $mysqli->prepare("UPDATE table_giohang SET order_status=5, cancel_requested=0 WHERE code_cart=? AND cancel_requested=1 AND order_status IN (1,2,3)");
    if ($stmt) {
        $stmt->bind_param('s', $code_cart);
        $stmt->execute();
        $stmt->close();
...
} elseif (isset($_GET['action']) && $_GET['action'] === 'reject_cancel' && isset($_GET['code'])) {
    $code_cart = $_GET['code'];
    $ensureCancelRequestedColumn();

    $stmt = $mysqli->prepare("UPDATE table_giohang SET cancel_requested=0 WHERE code_cart=? AND cancel_requested=1");
    if ($stmt) {
        $stmt->bind_param('s', $code_cart);
        $stmt->execute();
        $stmt->close();
